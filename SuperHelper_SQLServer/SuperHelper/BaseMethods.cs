...
namespace SuperHelper_SQLServer
{
    ...
    partial class BaseMethods
    {
        ...
        public static SqlParameter[] BaseMethod_ParamProcessed(string sql, List<object> pam_obj)
        {
            //1、找出sql语句中所有参数
            ...
            SqlParameter[] param = new SqlParameter[str_param.Count];  //定义用来访问数据库的SQLiteParameter参数数组
            #endregion
            ...
            //SqlParameter[] param = new SqlParameter[mc.Count];  //定义用来访问数据库的SqlParameter参数数组
            ...
            foreach (string str in str_param)
            {
                ...
                if (dic.ContainsKey(str))
                {
                    ...
                    param[index_param] = new SqlParameter("@" + str, dic[str]);
                }
                ...
                else
                {
                    ...
                    {
                        //这里需要注意：上面正则表达式第一组匹配项获取到的参数名是不带@的字符串，所以这里需要补上
                        param[index_param] = new SqlParameter("@" + str, obj[index_obj]);
                        index_obj += 1;
                    }
                    catch (Exception ex)
                    {
